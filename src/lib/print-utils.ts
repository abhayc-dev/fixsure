...
export const generateJobBill = (shop: Shop, job: JobSheet) => {
    ...
    let currentY = 30 + (addressLines.length * 4);
    doc.text(`Phone: ${shop.phone}`, pageWidth / 2, currentY, { align: 'center' });

    if (shop.gstNumber) {
        currentY += 5;
        doc.text(`GST No: ${shop.gstNumber}`, pageWidth / 2, currentY, { align: 'center' });
    }

    // Line Divider
    const headerBottom = currentY + 10;
    ...
    doc.text("Generated by RepairDesk - Repair Management System", 10, 285);
