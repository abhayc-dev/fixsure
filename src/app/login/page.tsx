...
export default function LoginPage() {
  ...
  // For Google Login flow requiring phone
  ...
  const handleCompleteGoogleSignup = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (res.success) {
            ...
            setError(res.error || "Failed to complete signup");
        }
    } catch (err) {
        setError("Something went wrong");
    } finally {
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-white/50 p-6 md:p-8 animate-in fade-in zoom-in duration-700">
          <div className="text-center">
            ...
            <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 mb-1">
              {googleUser ? "Secure Account" : mode === "login" ? "Welcome Back" : "Partner Signup"}
            </h1>
            ...
            {googleUser ? (
              <form onSubmit={handleCompleteGoogleSignup} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="w-full h-12 pl-11 pr-4 rounded-xl border border-slate-200 bg-slate-50/50 text-slate-900 placeholder:text-slate-400 focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all font-semibold text-sm"
                      value={phone}
                ...
                <button
                  ...
                  className="w-full h-12 rounded-xl bg-primary text-white font-bold shadow-lg shadow-orange-200 hover:shadow-orange-300 hover:-translate-y-0.5 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:translate-y-0"
                >
