...
export default function JobDetailsView({ job, shop, onBack }: { job: any, shop: any, onBack?: () => void }) {
    ...
    return (
        <div className="w-full h-full bg-white flex flex-col items-center p-8 overflow-y-auto">
            {/* Action Buttons */}
            <div className="w-full max-w-[800px] flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 sm:gap-0 mb-6 print:hidden">
                <button
                    onClick={handleBack}
                    className="flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-3 sm:py-2 rounded-lg font-medium transition-colors cursor-pointer w-full sm:w-auto"
                >
            ...
            <div className="w-full max-w-[800px] overflow-x-auto pb-6 custom-scrollbar">
                {/* THE INVOICE SHEET (Fixed Width content) */}
                <div ref={ticketRef} className="w-[800px] max-w-none sm:w-full bg-white p-8 border border-slate-200 shadow-sm print:shadow-none print:border-none text-black shrink-0 mx-auto">
